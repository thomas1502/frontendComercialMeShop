...
import { Top3ProductosCarrera } from '../../../functions/conexionesGraficas';
...
const SalesMonth = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const response = await Top3ProductosCarrera(dataFields);
  ...
  return (
    <DashCard title="Top 3 carreras con más productos">
      <div className="mt-3 position-relative" style={{ height: '300px', position: 'relative' }}>
        ...
        <div className="round-overlap sales" style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }}>
          <i className="bi bi-bag-fill text-muted" style={{ fontSize: '2rem', color: 'white' }}> </i>
        </div>
      </div>
      <div className="hstack gap-2 mt-2 pt-1 justify-content-left">
        <ul style={{ listStyle: 'none', textAlign: 'left', paddingLeft: 0 }}>
          {seriesSales.map((carrera) => (
            <li key={carrera.name}>
              <i className="bi bi-circle-fill fs-6 me-2" style={{ color: carrera.color }}></i>
              {carrera.name}: {carrera.data}
            </li>
          ))}
        </ul>
      </div>
